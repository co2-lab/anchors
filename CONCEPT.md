...
- **Estrutura de Projeto** — [`STRUCTURE.md`](./STRUCTURE.md). A planta da casa:
...
- **Planejamento** — [`PLANNING.md`](./PLANNING.md). A origem do *movimento*: semeia
...
- **Spec** — [`SPEC.md`](./SPEC.md). A origem da *verdade*: a âncora-base, o
...
- **Rastreabilidade** — [`TRACEABILITY.md`](./TRACEABILITY.md). A cola, em duas
  ...
  fincam raiz.
- **Propagação** — [`PROPAGATION.md`](./PROPAGATION.md). O motor: faz uma alteração
...
- **Qualidade** — [`QUALITY.md`](./QUALITY.md). Sem qualidade *medida*, o "bem
...
pilar próprio: a **Propagação**, especificada em [`PROPAGATION.md`](./PROPAGATION.md).
...
## 5.2 Duas réguas de decisão

Duas regras que valem ao decidir QUALQUER coisa neste framework — e que aparecem toda
vez que alguém pesa "o certo" contra "o barato".

### O esforço não decide o desenho

Levantar o custo de uma opção é honesto e necessário. Deixar o custo ESCOLHER não é.

Quando a opção correta implica trabalho grande (reescrever, dividir, migrar), o volume
é **informação**, não veto. Escolha o desenho certo e pague o preço; uma decisão errada
se reverte quando se percebe o erro, mas um desenho torto por preguiça fica — e vira o
alicerce de tudo que se constrói em cima.

> Na prática: ao apresentar alternativas, diga quanto cada uma custa **e** qual é a
> correta. Não recomende a mais barata *por ser* a mais barata.

### A complexidade do projeto não sobe para o framework

O Anchors oferece **mecanismo**; o projeto declara **conteúdo**. Quando um mecanismo
simples e universal não cobre um caso particular de um projeto, a pergunta certa é:
*isto é universal, ou é aquele projeto que está torto?*

Se for o segundo, **o projeto se adapta** — o engine não ganha uma heurística para
acomodar a bagunça de um consumidor. Heurística no núcleo é dívida que todo projeto
futuro paga, para resolver o problema de um só.

Exemplos da divisão em vigor:
- o engine sabe que existem **letras de tipo de regra**; o projeto declara quais
  (`rule_types`);
- o engine sabe que existem **obrigações transversais**; o projeto declara quais
  (`obligations`) — o Anchors não sabe o que é LGPD;
- o engine sabe que existem **camadas e regimes**; o projeto declara os seus;
- o engine sabe que uma função pode prometer um conjunto e devolver uma página; o projeto
  declara como se reconhece uma função e uma consulta na linguagem dele (`dialect`).

### O corolário: linguagem é conteúdo, não mecanismo

O último exemplo é o mais fácil de errar, e por isso merece a regra explícita: **um gate
nunca crava sintaxe de linguagem**. A verdade que ele confronta é universal ("quem promete
o conjunto não devolve a primeira página em silêncio"); o que muda por projeto é como se
reconhece uma função, um laço, um cursor, uma consulta. Isso é conteúdo — vem do
`anchors.yaml`, como tudo mais que é do projeto.

O custo de errar não é teórico, e é pior do que parece: **um dialeto errado não dá erro,
dá silêncio**. Medido — a primeira versão do gate `sibling-guard` casava `export function`
e ficou cega para 291 das 298 funções dos models de um projeto real (97,6% usavam `export
async function`). O gate rodou, não viu nada, e reportou ✓. **Gate cego é pior que gate
ausente**, porque ocupa a linha do relatório com uma aprovação.

Daí a regra de fechamento: sem o dialeto declarado, o gate responde **pendente** dizendo
qual campo falta — nunca ✓. E a mesma linha separa o que entra embutido do que não entra:
convenções **amplamente difundidas** (os prefixos `list`/`getAll`, os formatos JUnit, lcov
e Mutation Testing Elements) têm default no engine; nomes de **vendor** (`QueryCommand`,
do SDK de um provedor específico) nunca — esses o projeto declara.

**A regra vale para tudo que o Anchors EMITE, não só para o que ele lê.** Um artefato
gerado na linguagem errada é o mesmo erro visto do outro lado, e mais visível: um esqueleto
de teste com `describe/it` gravado num `.py` não é só inadequado — é sintaxe inválida, um
arquivo que não roda. A lista do que é do projeto, e não do framework:

| o que se emite | de onde vem |
|---|---|
| esqueleto de caso de teste | família da linguagem (`dialect.family`) |
| marcador de comentário do header | extensão do arquivo de destino |
| palavras-chave do Gherkin | `dialect.gherkin_language` (default `en`) |
| tag de regime do cenário | de-para `derived.regimes` do projeto |
| letras do código de cenário | vocabulário `rule_types` do projeto |

O último merece nota, porque tem o modo de falha mais perigoso: um regex de código preso
às letras canônicas torna **invisível** todo cenário de uma letra que o projeto declarou.
O gate não vê o cenário e reporta **verde sobre o que não conferiu**. Aconteceu — a letra
`I` (Invariant) foi declarada, dois regexes ficaram para trás, e nenhum teste pegou.
Sempre que um regex depender do vocabulário do projeto, ele precisa ser **reconfigurado na
carga da config**, nunca compilado com o default e esquecido.

E onde nada foi declarado, a saída é **um TODO visível, não um chute**: uma tag de regime
errada passa despercebida (nenhum gate a confronta), enquanto um `@TODO-tag-de-regime` é
corrigido na hora.
