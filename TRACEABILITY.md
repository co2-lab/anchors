...
> Ele pressupõe o mecanismo geral de [`CONCEPT.md`](./CONCEPT.md) — âncora, grafo,
...
```
spec.md   declara   →  CODEX-S01     (o requisito nasce com identidade)
feature   marca     →  @CODEX-S01     (a mesma identidade, outra forma)
teste     titula    →  CODEX-S01: …   (a mesma identidade, executável)
código    realiza   →  CODEX-S01      (a mesma identidade, no fonte)
```

### A quarta forma tem começo e fim: a região

As três primeiras formas são **delimitadas pela própria sintaxe** em que vivem: um
requisito na spec é uma linha; um cenário na feature vai até o próximo `Scenario`; um
caso de teste é um bloco fechado pela linguagem. O código era a exceção — o marcador
no fonte diz onde o requisito **começa** e nada diz onde ele **termina**.

A consequência é que a identidade no código é um *conjunto*, não um *intervalo*: sabe-se
que o arquivo realiza `CODEX-A03`, não onde. Isso basta para as perguntas de existência
("quem realiza este requisito?") e falha nas de mudança ("**qual** requisito mudou?").
Só se pode responder pelo arquivo inteiro — e um arquivo com catorze requisitos dá
catorze respostas quando a verdade é uma.

A delimitação é declarada por uma **região**, na sintaxe que os editores já colapsam:

```
// #region [CODEX-A03]: confirmar persiste o lançamento e fecha.
…
    // #region [CODEX-B05]: falha ao gravar avisa o usuário.
    …
    // #endregion [CODEX-B05]
…
// #endregion [CODEX-A03]
...
Três decisões, e cada uma responde a uma forma de apodrecer:

- **Reusa `#region`/`#endregion`** em vez de inventar par próprio. O par tem utilidade
  *para quem escreve* — o editor colapsa o bloco. Uma marcação que só o validador lê é
  burocracia, e burocracia apodrece; uma que serve ao autor é mantida por interesse dele.
- **O fecho repete o código.** Com fecho anônimo, aninhamento trocado é **indetectável**:
  a contagem de abre/fecha bate, e o intervalo medido é o errado — em silêncio, que é a
  pior forma de errar. Com o código no fecho, o gate confronta nome contra nome e aponta a
  linha. O `[...]` delimita a identidade sem ambiguidade com o texto da descrição.
- **A região é opcional.** Onde ela existe, a identidade tem intervalo; onde não existe,
  vale o arquivo — o comportamento antigo. Um projeto não precisa converter nada para
  continuar válido, e migra onde a precisão lhe paga. Exigir a conversão de tudo de uma
  vez é o tipo de mudança que ninguém revisa e que nasce desatualizada.

Nada disso pede que o Anchors **parseie** a linguagem: a região é uma anotação em
comentário, e a gramática de comentário por extensão já é fonte única no projeto. A
delimitação continua sendo *declarada pelo autor*, nunca inferida da estrutura do código
— inferir por indentação foi medido e reprovado (num arquivo JSX real, um comentário de
uma linha "engoliu" 489 linhas, porque indentação profunda em markup é aninhamento
visual, não escopo).
...
citando `LGNNX-S02`. A feature declara `@LOGIX-S02`; o gate procura um teste que cite
`LOGIX-S02`; o `LGNNX-S02` obsoleto **não casa**. Resultado: a feature parece
...
### A variante silenciosa: a citação que aponta para o vazio

O órfão clássico é o arquivo sem identidade — visível, porque falta alguma coisa. Existe
uma variante em que **nada falta**, e por isso ela não é vista: a spec **cita** o
requisito de uma unidade que não existe. `XXXXX-B07` numa nota, numa Tabela de
Dependências, numa referência cruzada — e `XXXX` não é dono de arquivo nenhum.

A citação parece rastreabilidade. Não é: é um ponteiro solto com aparência de vínculo. E o
custo é maior que o de um órfão comum, porque quem lê depois — pessoa ou agente — toma a
citação como **registro do que foi feito**. Medido: uma spec de schema afirmava *"Índices
que o schema criou (2026-08-11)"* e referenciava quatro códigos de specs que nunca
existiram; o arquivo que ela descreve não tinha uma linha da entidade. Todos os gates
verdes — a spec tinha código, header e seções, e o gate de dependências confronta símbolo
de código, não prosa.

O gate é `code-reference-valid`: todo código citado precisa existir no universo de
identidades do projeto. É barato e fecha a última porta pela qual uma âncora consegue
mentir sem que nada acuse.

### A identidade é DECLARADA, não inferida da ordem do texto

Corolário operacional, e a origem de um bug real: a identidade de um artefato é a que o
**header declara** (`code: XXXX`). Inferi-la do primeiro código de cenário que aparece no
corpo funciona na maioria dos casos e falha exatamente onde dói — quando a spec **cita
outra unidade antes de definir a sua**.

O efeito é silencioso e contamina tudo: a spec de um modelo que abria referenciando
`DTAXX-B11` entrava no mapa como **dona de `DTAX`**, e a partir daí todo gate relacional
passava a confrontar a unidade errada, sem nada indicar o desvio. Medido num projeto real:
**20 nós** com a identidade trocada — incluindo um guide que usava o código de outra
unidade como *exemplo* e por isso roubava a identidade dela.

Regra: onde houver identidade declarada, ela vence. A inferência é fallback para quem não
declara, nunca uma segunda fonte de verdade concorrente.
