...
func TestProjectGuideCobreAsPecasDaFase(t *testing.T) {
	casos := map[string]string{
		...
		"o descarte (o que dá valor ao INSIGHTS)": "Discarded",
		"a revisão de inconsistências":            "inconsistency review",
		"a entrevista roda na conversa":           "CONVERSATION",
		"uma pergunta por vez":                    "ONE question at a time",
		"ser opinativo":                           "OPINIONATED",
...
func TestProjectGuideTemAsCincoEtapasTecnicas(t *testing.T) {
	...
	for _, assunto := range []string{"indentation", "extensions", "editors", "paradigm", "co-location"} {
