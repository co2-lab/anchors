...
func newImpactCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "impact <file>",
		Short: "Impact analysis: what a change to this file reaches",
		Long: `Queries the map and shows, for a changed file, TWO dimensions:

  • Propagates to — the children that depend on it and need to be redone (the wave,
    going down; stops at @noPropagation nodes).
  • Validates against — the parents it must be confronted with (going up;
    a divergence here becomes an issue, not propagation).

Query only — it opens no issue and changes nothing.`,
	...
	cmd.Flags().StringVar(&mapPath, "map", "", "path to the map (default: <root>/anchors.graph.yaml)")
	return cmd
}
...
func printImpact(imp mapx.Impact) {
	...
	fmt.Printf("↓ Propagates to (%d) — they depend on this change, redo them:\n", len(imp.Propagate))
	if len(imp.Propagate) == 0 {
		fmt.Println("   (nothing — no child depends on it)")
	}
	for _, n := range imp.Propagate {
		fmt.Printf("   %s\n", n)
	}

	fmt.Printf("\n↑ Validates against (%d) — confront; a divergence becomes an issue:\n", len(imp.Validate))
	if len(imp.Validate) == 0 {
		fmt.Println("   (nothing — it is not governed by anyone)")
	}
