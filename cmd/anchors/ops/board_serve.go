...
func newBoardCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "board",
		Short: "The project board — published by the pipeline, or served live",
	}
...
func newBoardServeCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "serve",
		Short: "Bring up the local board, with LIVE state instead of the pipeline snapshot",
		Long: `Brings the board up on a local server, reading the current state from GitHub.

The published board is a SNAPSHOT: the pipeline runs, generates the board.json, publishes. Between one
run and the next what you see is the past — and in a project with many agents delivering,
minutes are enough for the snapshot to lie about who has which card.

  anchors board serve                 # http://localhost:7777
  anchors board serve --port 8080
  anchors board serve --interval 15s  # the floor between two reads

HOW IT STAYS FRESH. The read is INCREMENTAL: it asks GitHub what changed
since the last time, and the cost follows the CHANGE instead of the time. When the incremental
is not available — on the first load, without permission, or with the API refusing — it
FALLS BACK to the full sweep (fallback), which re-reads everything.

The difference matters because of the rate limit: it is 5000 calls/hour shared among all
the project's agents, and a full sweep costs ~15. Reloading every 30s without
incremental would be 1800/hour from the board alone.

The --interval is the FLOOR between two reads, not the frequency: if nothing changed, it does not
re-read. It serves to keep the board from overloading the API when a lot happens at once.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if repo == "" {
				r, err := repoAtual()
				if err != nil {
					return fmt.Errorf("discover the repository: %w\n\n"+
						"declare it with --repo <owner/name> if this directory is not a clone", err)
				}
				repo = r
			}
			...
			if err != nil {
				return fmt.Errorf("read the board HTML: %w", err)
			}
			...
			fmt.Printf("live board at http://%s\n", addr)
			fmt.Printf("  repository: %s\n", repo)
			fmt.Printf("  floor between reads: %s\n\n", intervalo)
			fmt.Println("Ctrl-C to stop.")
			return http.ListenAndServe(addr, mux)
		},
	}

	cmd.Flags().IntVar(&porta, "port", 7777, "local server port")
	cmd.Flags().DurationVar(&intervalo, "interval", 15*time.Second,
		"floor between two GitHub reads (not the frequency: with no change, it does not re-read)")
	cmd.Flags().StringVar(&repo, "repo", "", "owner/name (default: that of the current clone)")
	return cmd
}
...
func repoAtual() (string, error) {
	...
	if r == "" {
		return "", fmt.Errorf("`gh` did not return the repository")
	}
	return r, nil
}
...
func coletaCompleta(repo string) ([]byte, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("collect from GitHub: %w%s", err, func() string {
			if stderr != "" {
				return "\n  " + stderr
			}
			return ""
