...
func flagAnswers(cmd *cobra.Command, f *flagsInit) (initx.Respostas, error) {
	...
	if len(f.governs) > 0 {
		...
		for _, g := range f.governs {
			...
			if !ok {
				return r, fmt.Errorf("--governs expects GUIDE=tag1,tag2 (received %q)", g)
			}
...
func runInitNonInteractive(cmd *cobra.Command, root string, f *flagsInit, aceitarDefaults bool) error {
	...
	if !answeredSomething(r) && !aceitarDefaults {
		return emitJSON(map[string]any{
			...
			"escrito":           false,
			"perguntas":         qs,
			"como_responder": "pass the answers in flags (e.g.: --artifacts=spec,feature,test " +
				"--colocation) and call again; or --defaults to accept the defaults " +
	...
	if !initx.TudoAceito(status) {
		_ = emitJSON(map[string]any{
			"escrito":   false,
			"respostas": status,
			"erro":      "there are invalid answers — nothing was written",
		})
...
func nextStepAfter(root string, p *initx.Proposal) string {
	if initx.PrecisaDescobrir(root, p) {
		return "the DISCOVER phase did not happen: run `anchors guide project` and conduct the " +
