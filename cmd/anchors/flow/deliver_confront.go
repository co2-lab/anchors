...
func confrontDelivery(root string, files []string, unit string) {
	...
	if !confrontou {
		...
		fmt.Println("\n⚠ could not confront the declared files against what changed:")
		fmt.Println("  " + gitmeta.Explain(gitmeta.Check(root), "read the state of the working tree"))
		fmt.Println("  the delivery goes on, but nobody verified whether you did what you declared.")
	}
	if len(avisos) > 0 {
		fmt.Println("\n⚠ declared files that do NOT appear in the diff:")
		for _, a := range avisos {
			fmt.Printf("    %s\n", a)
		}
		fmt.Println("  Either you did not do what you declared, or you already committed — in any case, the")
		fmt.Println("  record is asserting something the disk does not confirm.")
	}
	if aviso := mutationNotMeasured(root, unit); aviso != "" {
		fmt.Println("\n⚠ " + aviso)
	}
	if linhas := redGates(root, files, unit); len(linhas) > 0 {
		fmt.Println("\n⚠ INFORMATIVE gates failing on the files of this delivery:")
		for _, l := range linhas {
			fmt.Printf("    %s\n", l)
		}
		fmt.Println("  Informative does not block promotion — and that is exactly why it goes")
		fmt.Println("  unnoticed. If you declared the opposite of what the gate says, check")
		fmt.Println("  before the reviewer checks for you.")
	}
}
...
func mutationNotMeasured(root, unit string) string {
	...
	return "this unit has a test and NO mutation signal ingested.\n" +
		"  Mutating by hand catches what you remember to mutate — and what escapes is exactly what\n" +
		"  you did not think of attacking (measured: 6 manual mutations, 6 killed, and even so a\n" +
		"  reviewer inverted a criterion and the tests stayed green). If the stack has a\n" +
		"  tool, run it and `anchors ingest --mutation <report>`."
}
