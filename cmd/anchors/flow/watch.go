...
func newWatchCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "watch",
		Short: "The background watcher: sees changes and QUEUES work",
		Long: `The watcher (PROPAGATION §6) observes the repo and, for each file that changes,
classifies the change and QUEUES a task in .anchors/tasks/ (with the suggested next
step). It does NOT execute nor call AI — whoever works pulls the task with 'anchors
next'. That is the inversion: the watcher only turns "it changed" into "there is work".

Runs in the BACKGROUND — the terminal stays free. Control it with the subcommands:
  anchors watch start    starts the watcher (daemonizes, returns the terminal)
  anchors watch status   is it running? since when? paused?
  anchors watch stop     shuts it down
  anchors watch pause    pauses (stops reacting, without shutting down)
  anchors watch resume   resumes
  anchors watch logs     shows what the watcher reported`,
	}
...
func newWatchStartCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Printf("watcher started in the background (pid %d)\n", pid)
			fmt.Printf("  logs:   anchors watch logs\n  status: anchors watch status\n  stop:   anchors watch stop\n")
			return nil
		},
	}
	addRootFlag(cmd)
...
func newWatchRunCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:    "run",
		Short:  "Run the watcher loop in the foreground (daemon internal use)",
		Hidden: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			root, err := absRootFlag(cmd)
			if err != nil {
	...
	addRootFlag(cmd)
	cmd.Flags().IntVar(&debounceMs, "debounce", 300, "debounce window in ms")
	return cmd
}
...
func newWatchStatusCmd() *cobra.Command {
	cmd := &cobra.Command{Use: "status", Short: "Watcher state",
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newWatchStopCmd() *cobra.Command {
	cmd := &cobra.Command{Use: "stop", Short: "Shut down the watcher",
		RunE: func(cmd *cobra.Command, args []string) error {
			root, _ := absRootFlag(cmd)
			if err := daemon.Stop(daemon.PathsFor(root)); err != nil {
				return err
			}
			fmt.Println("watcher shut down")
			return nil
		}}
	addRootFlag(cmd)
	return cmd
...
func newWatchPauseCmd() *cobra.Command {
	cmd := &cobra.Command{Use: "pause", Short: "Pause the watcher (without shutting it down)",
		RunE: func(cmd *cobra.Command, args []string) error {
			root, _ := absRootFlag(cmd)
			p := daemon.PathsFor(root)
			...
			fmt.Println("watcher paused (resume to continue)")
			return nil
		}}
	addRootFlag(cmd)
	return cmd
...
func newWatchResumeCmd() *cobra.Command {
	cmd := &cobra.Command{Use: "resume", Short: "Resume the paused watcher",
		RunE: func(cmd *cobra.Command, args []string) error {
			root, _ := absRootFlag(cmd)
...
func newWatchLogsCmd() *cobra.Command {
	cmd := &cobra.Command{Use: "logs", Short: "Show the watcher log",
		RunE: func(cmd *cobra.Command, args []string) error {
			root, _ := absRootFlag(cmd)
			data, err := os.ReadFile(daemon.PathsFor(root).Log)
			if err != nil {
				return fmt.Errorf("no log (has the watcher ever run?): %w", err)
			}
...
func runWatchLoop(root string, cfg *config.Config, g *mapx.Graph, debounce time.Duration) error {
	...
	fmt.Printf("anchors watch — watching %d directories under %s\n", added, root)
	...
	if unidade, ok := changeDelivered(root, rel); ok {
		...
		if deliveryPlan(root, rel) {
			enqueueTask(root, rel, "change", "review-plan",
				"a PLAN was delivered — REVIEW THE WHOLE (`anchors work review-plan --for "+
					unidade+"`). The per-unit review already guaranteed each piece; this one guarantees the "+
					"SEAM: what one layer promises and another does not deliver, the rule that "+
					"contradicts the one above, the duty that passes structurally in each piece and "+
					"does not work as a whole. Measured: 4 of 6 findings of a real review "+
					"crossed units, with each piece correct on its own.")
			return
		}
...
func enqueueTask(root, rel, kind, next, reason string) {
	...
	if err != nil {
		fmt.Printf("● %s [%s] — error while queueing: %v\n", rel, kind, err)
		return
	}
	if !created {
		fmt.Printf("● %s [%s] — already in the queue (%s)\n", rel, kind, next)
		return
	}
	fmt.Printf("● %s [%s] → task queued: %s (%s)\n", rel, kind, t.ID, next)
	if n, _ := queue.PendingCount(root); n > 0 {
		fmt.Printf("   queue: %d task(s) — a worker can pull with `anchors next`\n", n)
	}
}
