...
var writeInitialProgress = WriteInitialProgress
...
func WriteInitialProgress(planoPath, conteudoPlano, codigo string) (string, error) {
	...
	if _, err := os.Stat(destino); err == nil {
		return "", fmt.Errorf("%s already exists — progress is state, and I do not overwrite it", destino)
	}

	var b strings.Builder
	b.WriteString("<!-- anchors:progress — the STATE of plan `" +
		filepath.Base(planoPath) + "`.\n\n")
	b.WriteString("This file stays OUTSIDE THE MAP on purpose: it exists to CHANGE, and a\n")
	b.WriteString("file that changes by nature cannot be confronted by the gates that\n")
	b.WriteString("charge a justification for change. The plan next to it is the DECISION;\n")
	b.WriteString("altering it has to mean the decision changed.\n\n")
	b.WriteString("Mark `[x]` here, never in the plan.\n-->\n\n")
	b.WriteString("# Progress — " + codigo + "\n\n")

	fases := planPhases(conteudoPlano)
	if len(fases) == 0 {
		b.WriteString("TODO: the plan does not declare phases yet. When it does, add one\n")
		b.WriteString("section per phase here, with one item per seeded spec.\n")
	}
	for _, f := range fases {
		...
		b.WriteString("\n\n- [ ] TODO: one item per spec this phase seeds\n\n")
	}
...
func NewProgressCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Create the `-progress.md` of an existing plan (the STATE, outside the map)",
		Long: `Creates the progress companion of a plan that already exists.

A plan is DECISION; progress is STATE. While the phase checkboxes lived in the
plan, marking ` + "`- [x]`" + ` was ALTERING the plan — and that charged a revision to whoever only
finished a phase, besides knocking down the judgment of the spec that the phase
delivered (the edge stamp holds the rev of both ends).

The ` + "`anchors new plan`" + ` already creates the companion. This command is for the plans that
were born before:

  anchors new progress --for plans/0002-plataforma.md

It does not overwrite: the file holds state, and rewriting it would erase what was
already recorded.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if para == "" {
				return fmt.Errorf("provide the plan with --for (e.g.: `anchors new progress --for plans/0002-x.md`)")
			}
			...
			codigo := common.CodeDoHeaderSpec(string(conteudo))
			if codigo == "" {
				return fmt.Errorf("plan %s does not declare `code:` in the @anchors header — "+
					"without it the progress would be born without identity", para)
			}
			prog, err := WriteInitialProgress(planoPath, string(conteudo), codigo)
			if err != nil {
				return err
			}
			fmt.Printf("✓ %s created (the STATE of plan %s; mark `[x]` here, never in the plan)\n",
				common.RelTo(absRoot, prog), codigo)
			if fases := planPhases(string(conteudo)); len(fases) > 0 {
				fmt.Printf("  %d phase(s) of the plan, one section for each\n", len(fases))
			}
			return nil
		},
	}
	cmd.Flags().StringVar(&root, "root", ".", "project root")
	cmd.Flags().StringVar(&para, "for", "", "the plan whose progress will be created")
	return cmd
}
