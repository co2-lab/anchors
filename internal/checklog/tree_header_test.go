...
func TestCabecalhoRelataContagemReal(t *testing.T) {
	...
	if h := Header("c", "h", "a", 0, quando); !strings.Contains(h, "tree: clean") {
		t.Errorf("0 sujos É árvore limpa: %s", h)
	}
	if h := Header("c", "h", "a", 1, quando); !strings.Contains(h, "1 modified file") {
		t.Errorf("1 sujo: %s", h)
	}
	if h := Header("c", "h", "a", 5, quando); !strings.Contains(h, "5 modified file(s)") {
