...
func StepsFrom(origem, destino int) ([]Step, error) {
	...
	for _, p := range steps {
		...
		if p.To != esperado {
			return nil, fmt.Errorf("missing the migration step that produces format %d "+
				"(the next registered one is %d) — without it the file would get the new "+
				"number with the old content", esperado, p.To)
		}
		out = append(out, p)
		esperado++
	}
	if esperado != destino+1 {
		return nil, fmt.Errorf("missing the migration step that produces format %d", esperado)
	}
	return out, nil
}
