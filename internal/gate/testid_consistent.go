...
func checkTestIDCoherent(content string, n mapx.Node, root string, g *mapx.Graph, cfg *config.Config) (Verdict, string) {
	if n.Kind != mapx.KindSpec {
		return Skip, i18n.T("gate.testid_consistent.skip_not_spec")
	}
	if g == nil {
		return pendingNoMap()
	}
	...
	if attr == "" {
		...
		return Skip, i18n.T("gate.testid_consistent.skip_no_test_handle")
	}
	...
	if arquivo == "" {
		return Skip, i18n.T("gate.testid_consistent.skip_no_code")
	}
	...
	if len(expostos) == 0 && len(declarados) == 0 {
		...
		// apresentação pura.
		return Skip, i18n.T("gate.testid_consistent.skip_no_handles_exposed", attr)
	}
	...
	for _, id := range universo {
		...
		linhas = append(linhas, i18n.T("gate.testid_consistent.line_status",
	...
	sort.Strings(linhas)
	return Fail, i18n.T("gate.testid_consistent.fail_inconsistent",
