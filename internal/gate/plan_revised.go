...
// topNoticeRE casa o aviso no topo do documento.
func topNoticeRE() *regexp.Regexp {
...
func checkPlanRevised(content string, n mapx.Node, root string, g *mapx.Graph, cfg *config.Config) (Verdict, string) {
	if n.Kind != mapx.KindPlan {
		return Skip, i18n.T("gate.plan_revised.skip_not_plan")
	}
	if g == nil {
		return Pending, i18n.T("gate.plan_revised.pending_no_map")
	}
	...
	if len(revisores) == 0 {
		...
		if len(ausentes) > 0 {
			return Fail, i18n.T("gate.plan_revised.fail_revises_target_missing",
	...
	if !topNoticeRE().MatchString(docTop(content)) {
		return Fail, i18n.T("gate.plan_revised.fail_revised_without_notice",
			strings.Join(revisores, ", "), revisores[0])
	}
	return Pass, ""
}

// docTop devolve o começo do documento — onde o aviso tem de estar para ser visto.
//
...
func docTop(content string) string {
