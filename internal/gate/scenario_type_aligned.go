...
func checkScenarioTypeAligned(content string, n mapx.Node, _ string, _ *mapx.Graph, cfg *config.Config) (Verdict, string) {
	...
	if cfg == nil || len(cfg.RuleTypes) == 0 {
		return Skip, i18n.T("gate.scenario_type_aligned.skip_no_rule_types")
	}
	...
	if !temMapa {
		return Skip, i18n.T("gate.scenario_type_aligned.skip_no_tags_map")
	}

	cenarios := parseFeatureScenarios(content)
	if len(cenarios) == 0 {
		return Skip, i18n.T("gate.scenario_type_aligned.skip_no_scenarios_with_code")
	}
	...
	for _, sc := range cenarios {
		...
		for _, tag := range sc.Tags {
			...
			achados = append(achados, i18n.T("gate.scenario_type_aligned.finding_item",
				sc.Code, letra, nome, strings.Join(declaradas, "/"), shortenText(sc.Title, 48)))
		}
	}
	if len(achados) == 0 {
		return Pass, ""
