...
> dialeto de interface (um app mobile com backend próprio pode ter `screen` no app
> e `handler`/`trigger` no backend) — são tags distintas, mesma régua. Ver
> `SPEC_TYPES.md` §2.5.
...
está documentada em [`PROJECT_STRUCTURE.md`](https://github.com/co2-lab/anchors/blob/main/PROJECT_STRUCTURE.md):
- As camadas declaradas em `anchors.yaml`;
- A divisão do CLI `cmd/anchors/` por domínios (`flow`, `quality`, `governance`, `mapcmd`, `ops`);
- O núcleo e os pacotes de suporte em `internal/`;
