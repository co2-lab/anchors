...
### Demarcation in source code: Regions and Function Hashes (`#region [CODE]`)

While a spec delimits requirements by line, a feature by `Scenario`, and a test by an `it()` or `test()` block, source code used to be the exception: marking indicated where a requirement began, but not where it ended. Without demarcation, identity in code was a fuzzy set ("this file implements CODEX-A03") rather than a discrete line interval.

The practical side effect was **false staleness**: editing a single line or helper at the top of a file invalidated all cached test results across the entire file, even those covering completely untouched functions.

To solve this, Anchors implements **Declared Code Regions with Content Hashing**:

```typescript
// #region [MLETX-A03]: confirm persists entry and closes modal
export async function persistEntry(item: Entry): Promise<void> {
  await db.put(item);
  closeModal();
}
// #endregion [MLETX-A03]
...
* **Region Hash Calculation**: Anchors computes a SHA-256 hash (truncated to 12 hex characters) exclusively covering the lines between `#region` and `#endregion`.
* **Change Isolation**: Changes made outside the region **do not alter its hash**. If you edit a neighboring function `MLETX-A01`, region `MLETX-A03` remains identical and any test evidence covering it remains valid (does not become *stale*).
* **Language Agnostic**: Uses standard `#region`/`#endregion` syntax (natively recognized and collapsible in editors like VS Code and JetBrains) across any comment marker (`//`, `#`, `--`, `/* */`, `<!-- -->`).
* **Pairing Enforced**: The built-in gate `region-pair-honored` ensures every `#region` has a matching `#endregion` with the identical code identifier.

### Evidence Freshness: How Anchors detects Stale Tests

When tests run, `anchors ingest` ingests test reports (JUnit, LCOV, etc.) and stamps the test node:
1. `AtRev`: The revision of the test file itself at run time.
2. `ClosureRev`: The revisions and hashes of **all code units and regions** the test reaches (the dependency closure).

If any code unit or region in the closure changes, the **`evidence-fresh`** gate immediately reports that the test evidence has become **stale** (expired): the test was green in the past, but has not proven itself against today's code revisions. The gate blocks or flags until the suite is re-run.
