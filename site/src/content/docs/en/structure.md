...
## 9. The Blueprint of this Repository (Anchors)

The concrete implementation of this pillar's principles for the Anchors codebase
is documented in [`PROJECT_STRUCTURE.md`](https://github.com/co2-lab/anchors/blob/main/PROJECT_STRUCTURE.md):
- Layers declared in `anchors.yaml`;
- Domain-based organization of the CLI in `cmd/anchors/` (`flow`, `quality`, `governance`, `mapcmd`, `ops`);
- The core and support packages in `internal/`;
- Layer boundaries and dependency direction rules.
