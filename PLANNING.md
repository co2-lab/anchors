...
> mecanismo de [`CONCEPT.md`](./CONCEPT.md) — âncora (na metáfora da escalada),
...
## 6.1 O ciclo de vida do plano: três estados, três pastas

O plano é o único artefato que ORIGINA trabalho — e por muito tempo foi o único que
ninguém confrontava antes de executar. O custo disso é assimétrico: um defeito numa spec
afeta uma unidade; um defeito no plano se espalha por todas as que ele semeia.

Medido: um item escrito como *"nasce SE o mobile chamar via API Gateway; senão marcar
fora de escopo"* fez o agente executor **decidir sozinho**. Ele acertou — e ninguém tinha
pedido que decidisse. Uma decisão de produto virou escolha de implementação, e uma fase
inteira nasceu sobre uma premissa que não passou por quem devia.

O plano tem três estados, e **o estado é a PASTA** — a mesma mecânica das issues
(`todo/doing/done`) e da fila (`pending__`/`claimed__`): mover é mudar de estado, e o
estado se lê sem abrir o arquivo.

| pasta | estado | o que o watcher faz |
|---|---|---|
| `plans/review/` | em revisão | enfileira **review**, nunca execução |
| `plans/` | vivo | semeia trabalho (`specify`) |
| `plans/done/` | histórico | nada — camada reconhecida |

**Por que o estado precisa existir, e não basta revisar "quando o plano muda":** o watcher
vê *"um arquivo mudou"* e nada mais. Sem separar os estados, ele não distingue "o plano
nasceu" de "o plano foi editado durante a execução" — e marcar um item como feito
dispararia um review novo a cada vez, até o passo virar ruído e alguém desligá-lo. O
review que ninguém lê é pior que review nenhum: ocupa a linha do relatório.

**Promover é um gesto, não um campo.** Mover de `plans/review/` para `plans/` é o que diz
"revisei e aprovo" — não há status a manter em dia, nem checkbox que possa mentir.

**E `plans/done/` é histórico, não roteiro** (CONCEPT §6, o plano vivo e o histórico): a
verdade dele é *datada* ("isto foi feito"), não *atual*. Por isso é camada RECONHECIDA —
